...
def myreader(fileName,detector,nf1,nf2,scan="none",skip=1):
    ...
    :param detector: str detector name in h5 file
    ...
    #Getting selun data
    if detector=='selun':
        print("Reading selun data")
        data = selun_data(f,nf1,nf2)#read selun data
        f.close()
...
def p10_eiger4m_event_GPU_datan(fileName,detector,nf1,nf2,mask,scan,thr=20,frc=0.15,skip=1):
    """ Read a P10 HDF5 master file using h5py, numpy and eigercompress 
    ...
    :param scan: str scan number 
    :param thr: int uper threshold to cut hot pixels
    ...
    if n_frames < 2**16:
        ...
        tms = np.zeros((ll,max_e),np.uint32)
        cnt = np.ravel(np.zeros((ll,),np.uint32))
    ...
    for i in dict(data):
        datas = data[i]
        nf, xf,yf = datas.shape
        #if frame_counter >= nf1:
        for i_n in range(0,nf,skip):   
            if frame_counter >= nf1:
                fr = np.ravel(datas[i_n,:,:])
                ...
                if it >= max_e:
                    ...
                    if cntm == max_e-1:
                        ...
                        tms = np.concatenate((tms,np.zeros((ll,nmax_e-max_e),np.uint32)),axis=1)
                        max_e = nmax_e+0
...
def id10_eiger4m_event_GPU_datan(fileName,detector,nf1,nf2,mask,scan,thr=20,frc=0.15,skip=1):
...
def h5writer(fileName,data):
    ...
    n2xentry.attrs['default'] = u'image_plot'
...
def selun_data(f,nf1,nf2):
    '''Read a selun HDF5 master file using h5py and numpy'''
    '''Provide just file identifyer of an open file''' 
    
    datas = f['/entry/data']
    nframes = 0  
    n_frames, nx, ny = list(datas.values())[0].shape 
    data = numpy.zeros((nf2-nf1,nx,ny),list(datas.values())[0].dtype)
    current_frame = 0 
    for i in dict(datas):
        n_frames, nx, ny = datas[i].shape
        for nn in range(n_frames):
             if (current_frame >= nf1) & (current_frame<=nf2):
                 data[nframes,:,:] = datas[i][nn,:,:]
                 nframes += 1
             current_frame += 1  
             if current_frame >= nf2:
                 break  
        if current_frame >= nf2:
            break         
    return data
